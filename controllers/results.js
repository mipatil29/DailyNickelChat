module.exports = function(async, Groups, Users){
    return {
        ...
        postResults: function(req, res){
            async.parallel([
                function(callback){
                    const regex = new RegExp((req.body.community), 'gi');
                    
                    Groups.find({'community':regex}, (err, result) => {
                       callback(err, result); 
                    });
                    
                    /*Groups.find({'$or': [{'community':regex}, {'name': regex}]}, (err, result) => {
                       callback(err, result); 
                    });
                    */
                ...
                res.render('results', {title: 'Daily Nickel Chat - Results', user: req.user, chunks: dataChunk});
            })
        }
